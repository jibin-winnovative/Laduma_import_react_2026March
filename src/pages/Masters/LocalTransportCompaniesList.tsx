...
import { Truck, Plus, Search, Filter, Eye, Edit2 } from 'lucide-react';
...
import { LocalTransportCompanyForm } from './LocalTransportCompanyForm';
import { ViewLocalTransportCompany } from './ViewLocalTransportCompany';

export const LocalTransportCompaniesList = () => {
  const [companies, setCompanies] = useState<LocalTransportCompany[]>([]);
  const [loading, setLoading] = useState(true);
  const [summary, setSummary] = useState({
    totalTransportCompanies: 0,
    activeTransportCompanies: 0,
    totalServiceAreas: 0,
  ...
  const fetchSummary = async () => {
    try {
      const data = await localTransportCompaniesService.getSummary();
      setSummary(data);
    } catch (error) {
  ...
  const fetchCompanies = async () => {
    try {
      setLoading(true);
      const response = await localTransportCompaniesService.getList({
        page: currentPage,
        pageSize,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[var(--color-text)]">Local Transport Companies</h1>
          <p className="text-[var(--color-text-secondary)] mt-1">
            Manage local transport company master data
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-[var(--color-text)] mt-1">
                {summary.totalTransportCompanies}
              </p>
            </div>
            <div className="p-2 sm:p-3 bg-blue-100 rounded-lg">
              <Truck className="w-5 h-5 sm:w-6 sm:h-6 text-[var(--color-primary)]" />
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-[var(--color-success)] mt-1">
                {summary.activeTransportCompanies}
              </p>
            </div>
            <div className="p-2 sm:p-3 bg-green-100 rounded-lg">
              <Truck className="w-5 h-5 sm:w-6 sm:h-6 text-[var(--color-success)]" />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">Service Areas</p>
              <p className="text-xl sm:text-2xl font-bold text-[var(--color-text)] mt-1">
                {summary.totalServiceAreas}
              </p>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className="px-4 py-8 text-center text-gray-500">
                    No local transport companies found
                  </td>
                </tr>
              ) : (
                companies.map((company) => (
                  <tr key={company.localTransportCompanyId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleView(company.localTransportCompanyId)}
                        ...
                        <button
                          onClick={() => handleEdit(company.localTransportCompanyId)}
