...
import { Plus, Search, Eye, Edit2, Receipt, CheckCircle, XCircle } from 'lucide-react';
import { addonChargesService, AddonCharge } from '../../services/addonChargesService';
import { Button } from '../../components/ui/Button';
import { Card } from '../../components/ui/Card';
import { AddonChargeForm } from './AddonChargeForm';
import { ViewAddonCharge } from './ViewAddonCharge';

export const AddonChargesList = () => {
  const [charges, setCharges] = useState<AddonCharge[]>([]);
  ...
  const fetchSummary = async () => {
    try {
      const allData = await addonChargesService.getAll();
  ...
  const fetchList = async () => {
    try {
      setLoading(true);
      const response = await addonChargesService.getList({
        page: currentPage,
        pageSize,
        searchTerm: appliedSearchTerm || undefined,
        isActive: appliedStatus === '' ? undefined : appliedStatus === 'active',
      });

      if (Array.isArray(response)) {
        let filtered = response as AddonCharge[];
      ...
      console.error('Failed to fetch addon charges:', error);
      setCharges([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[var(--color-text)]">Addon Charges</h1>
          <p className="text-[var(--color-text-secondary)] mt-1">
            Manage addon charge master data
          </p>
        </div>
        <Button
        ...
        >
          ...
          <span className="hidden sm:inline">Add Addon Charge</span>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-gray-500">
                    No addon charges found
                  </td>
                </tr>
              ) : (
                charges.map((item) => (
                  <tr key={item.addonChargeId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleView(item.addonChargeId)}
                        ...
                        <button
                          onClick={() => handleEdit(item.addonChargeId)}
