...
import { FileText, Plus, Search, Filter, Eye, Edit2, AlertCircle, Clock } from 'lucide-react';
import { importDocMastersService, ImportDocMaster } from '../../services/importDocMastersService';
import { Button } from '../../components/ui/Button';
import { Card } from '../../components/ui/Card';
import { ImportDocMasterForm } from './ImportDocMasterForm';
import { ViewImportDocMaster } from './ViewImportDocMaster';

const CATEGORIES = ['All', 'LOA', 'Test Reports/EE REPORT', 'Levy Declaration'];

export const ImportDocsPage = () => {
  const [documents, setDocuments] = useState<ImportDocMaster[]>([]);
  ...
  const fetchSummary = async () => {
    try {
      const data = await importDocMastersService.getSummary();
      setSummary(data);
    } catch (error) {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const response = await importDocMastersService.getList({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[var(--color-text)]">Import Documents</h1>
          <p className="text-[var(--color-text-secondary)] mt-1">
            Manage import document master data
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">Total Documents</p>
        ...
        <Card className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">Active Documents</p>
              <p className="text-xl sm:text-2xl font-bold text-[var(--color-success)] mt-1">
                {summary.activeDocuments}
              </p>
            </div>
            <div className="p-2 sm:p-3 bg-green-100 rounded-lg">
              <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-[var(--color-success)]" />
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs sm:text-sm text-[var(--color-text-secondary)]">Categories</p>
              <p className="text-xl sm:text-2xl font-bold text-[var(--color-text)] mt-1">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                documents.map((doc) => (
                  <tr key={doc.importDocMasterId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-[var(--color-text)]">
                      {doc.categoryDeclaration}
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      {renderExpiryDate(doc.expiryDate)}
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleView(doc.importDocMasterId)}
                        ...
                        <button
                          onClick={() => handleEdit(doc.importDocMasterId)}
      ...
      {showEditForm && selectedDocumentId && (
        <ImportDocMasterForm
          mode="edit"
